...
class RolloutStorage(object):
    ...
    def reset(self):
        ...
        if not isinstance(self.action_space, list):
            ...
            action_shape = 0
            for a_s in self.action_space:
                if isinstance(a_s, Discrete):
                    action_shape += 1
                elif isinstance(a_s, MultiBinary):
                    action_shape += a_s.shape[0]
                else:
                    action_shape += a_s.shape[0]

        self.actions = torch.zeros(self.num_steps, self.num_processes, action_shape)
        if self.action_space.__class__.__name__.find('Discrete') != -1:
    ...
    def feed_forward_generator(self,
        ...
        sampler = BatchSampler(
            ...
            SubsetRandomSampler(torch.where(self.bad_masks[:-1].view(-1) != 0)[0].tolist()),
            mini_batch_size,
            drop_last=False)
